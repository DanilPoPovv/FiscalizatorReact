...
export default function SearchBox({ columns, onSearch, itemCount }) {
  ...
  const criteriaChangedAfterLastSearch = useRef(false);
  function filteredSearch() {
    if (!criteriaChangedAfterLastSearch.current && hasSearch) {
      return;
    }
    if (!hasAnySearchCriteria && hasSearch) {
    ...
    if (hasAnySearchCriteria) {
      onSearch({ newFilters: values });
      setHasSearched(true);
      criteriaChangedAfterLastSearch.current = false;
      return;
    }
    setHasSearched(false);
  }
  ...
  function changeSearchFilters(criteria, newValue) {
    setValues(prev => ({ ...prev, [criteria.field]: newValue }));
    criteriaChangedAfterLastSearch.current = true;
  }
  return (
          ...
          <>
            {searchCriterias.map(criteria => (
              <InputRow
                ...
                onChange={(e) =>
                  changeSearchFilters(criteria, e.target.value)
                }
              />
            ))}
